...
import { FaHashtag, FaGlobe, FaLock, FaCircle, FaSignOutAlt } from 'react-icons/fa';

const Sidebar = ({ 
    isSidebarVisible, darkMode, getRecentChatUsers, activeRoom, handleStartDM, 
    handleOpenProfile, unreadCounts, onlineUsers, allGroups, handleSwitchRoom, 
    user, setUser 
}) => {
    const recentUsers = getRecentChatUsers();
    ...
    return (
        <div className={`flex flex-col border-r transition-all duration-300 ${isSidebarVisible ? 'w-64' : 'w-0 overflow-hidden'} ${darkMode ? 'bg-[#1e293b]/50 backdrop-blur-xl border-white/5' : 'bg-[#f2f3f5]'}`}>
            ...
            <div className="flex-1 p-2 mt-2 overflow-y-auto space-y-6 font-bold scrollbar-hide">
                ...
                {/* Tin nhắn trực tiếp */}
                <div>
                    <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest px-2 mb-3 italic opacity-70">Tin nhắn trực tiếp</p>
                    {recentUsers.length === 0 ? (
                        <div className="px-3 py-2 bg-black/5 dark:bg-white/5 rounded-xl border border-dashed border-white/10 text-center">
                             <p className="text-[9px] text-gray-600 uppercase font-black">Trống hội thoại</p>
                        </div>
                    ) : (
                        recentUsers.map(f => (
                            <div key={f} onClick={() => handleStartDM(f)} className={`p-2.5 rounded-lg flex items-center gap-3 cursor-pointer mb-1 relative transition-all ${activeRoom.name === f && activeRoom.isDM ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'hover:bg-white/5 text-gray-400'}`}>
                                <div className="relative shrink-0" onClick={(e) => { e.stopPropagation(); handleOpenProfile(f); }}>
                                    <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-white text-xs font-bold uppercase overflow-hidden border border-white/10 shadow-inner">
                                        {onlineUsers[f]?.avatar ? <img src={onlineUsers[f].avatar} className="w-full h-full object-cover" alt="avt" /> : f.substring(0,2).toUpperCase()}
                                    </div>
                ...
                {/* Kênh cộng đồng */}
                <div>
                    <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest px-2 mb-2 italic">Kênh cộng đồng</p>
                    <div onClick={() => handleSwitchRoom({id:'chung', name:'Chung'})} className={`p-2.5 rounded-lg flex items-center gap-3 cursor-pointer mb-1 relative transition-all ${activeRoom.id === 'chung' ? 'bg-[#5865f2] text-white shadow-lg font-black' : 'hover:bg-white/5 text-gray-400'}`}>
                        <FaHashtag size={14}/> <span className="text-sm">chung</span>
                    </div>
                    {allGroups.filter(g => g.isPublic).map(g => (
                        <div key={g.groupId} onClick={() => handleSwitchRoom({id:g.groupId, name:g.groupName})} className={`p-2.5 rounded-lg flex items-center gap-3 cursor-pointer mb-1 relative transition-all ${activeRoom.id === g.groupId ? 'bg-[#5865f2] text-white shadow-lg' : 'hover:bg-white/5 text-gray-400'}`}>
                            <FaGlobe size={12} className="opacity-60"/> <span className="text-sm truncate">{g.groupName}</span>
                        </div>
                    ))}
                </div>
                ...
                {/* Kênh riêng tư */}
                <div>
                    <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest px-2 mb-2 italic">Kênh riêng tư</p>
                    {allGroups.filter(g => !g.isPublic && (g.members?.includes(user.username) || g.owner === user.username || user.role === 'admin')).map(g => (
            ...
            <div onClick={() => handleOpenProfile(user.username)} className={`h-16 flex items-center px-3 cursor-pointer border-t border-white/5 transition-colors shrink-0 ${darkMode ? 'bg-[#020617]' : 'bg-[#ebedef]'}`}>
                <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-black shadow-lg overflow-hidden border border-white/20">
                    {user.avatar ? <img src={user.avatar} className="w-full h-full object-cover" alt="me" /> : user.displayName.substring(0,2).toUpperCase()}
                </div>
