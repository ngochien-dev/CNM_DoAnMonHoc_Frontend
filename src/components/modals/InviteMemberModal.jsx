            ...
            <div className={`w-[420px] rounded-[40px] p-8 shadow-2xl border ${darkMode ? 'bg-slate-900 border-white/10 text-white' : 'bg-white border-gray-100 text-slate-800'}`}>
                ...
                <div className="max-h-[300px] overflow-y-auto space-y-2 scrollbar-hide">
                    {(user.friends || []).filter(f => {
                        ...
                        const grp = allGroups.find(g => g.groupId === activeRoom.id);
                        const isPending = grp && (grp.pendingRequests || []).includes(f);
                        return (
                            <div key={f} className={`flex items-center justify-between p-3 rounded-2xl border transition-all ${darkMode ? 'bg-white/2 border-white/5 hover:bg-white/5' : 'bg-slate-50 border-gray-100 hover:bg-slate-100'}`}>
                                ...
                                {isPending ? (
                                    <span className="text-[10px] text-amber-500 bg-amber-500/10 px-3 py-1.5 rounded-xl font-black uppercase tracking-wider border border-amber-500/20 shadow-inner italic animate-pulse">Chờ duyệt</span>
                                ) : (
                                    <button
                                        onClick={async () => {
                                            try {
                                                const res = await api.post('/groups/invite', { groupId: activeRoom.id, targetUsername: f });
                                                if (res.data && res.data.joined === false) {
                                                    toast(`Đã gửi yêu cầu mời @${f}. Chờ phê duyệt!`, { icon: '⏳', style: { borderRadius: '10px', background: '#333', color: '#fff' } });
                                                } else {
